...
hpstr-jekyll-theme/
├── _includes
|    ├── blog-browser-upgrade.html       # prompt to upgrade browser on < IE8
|    ├── blog-footer.html                # site footer
|    ├── blog-head.html                  # site head
|    ├── blog-navigation.html            # site navigation
...
Change your name, and avatar photo (200x200 pixels or larger), email, and social networking URLs. If you want to link to an external image on Gravatar or something similar you'll need to edit the path in `blog-head.html` since it assumes it is located in `/images`.
...
For the most part you can leave these as is since the author/owner details are pulled from `_config.yml`. That said you'll probably want to customize the copyright stuff in `blog-footer.html` to your liking.
...
Twitter cards make it possible to attach images and post summaries to Tweets that link to your content. Summary Card meta tags have been added to `blog-head.html` to support this, you just need to [validate and apply your domain](https://dev.twitter.com/docs/cards) to turn it on.
